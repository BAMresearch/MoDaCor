...
- Add ``pipeline.attach_tracer_event(node, tracer, include_rendered_trace=True)`` inside the execution loop if you want
  to export the trace alongside the configuration.
